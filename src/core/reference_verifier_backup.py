...
class EmploymentVerifier:
    ...
    def verify_employment(
        ...
        # Fallback to email if phone failed or unavailable
        if hr_email:
            logger.info(f"Attempting email verification to {hr_email}")
            result = self._verify_via_email(employment, hr_email)
            
            if result.success:
                logger.info("Email verification successful")
                return result
            
            logger.warning(f"Email verification failed: {result.error_message}")
